...
class FlowDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.dataset_type = (
            dataset_type  # train, test, val  check if in allowed dataset types
        )
        ...
        self.transform = (
            transform  # if transform is not None else (lambda x, y: (x, y))
        )
        self.fcm_sample_type = load_type_dynamically_from_fqn(
            flowsample_config.sample_type
        )
        ...
        if self.enable_standardization:
            self.strdhandler = StandardizationHandler(
                self.preload_dir, self.dataset_type, self.flowsample_config.markers
            )
            self.strdhandler.prepare_stats(self.filepaths)

    @classmethod
    def init_from_config(cls, config: DataLoaderConfig, dataset_type: str):

        if not isinstance(config, DataLoaderConfig):
            raise TypeError(
                f"config must be from Type 'DataLoaderConfig' given {type(config)}"
            )
        ...
        if config.transforms_config is not None:
            transform_stack = TransformStack(
                [
                    init_ftn(config_tform)
                    for config_tform in config.transforms_config.transforms_list
                ],
