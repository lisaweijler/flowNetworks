...
class FlowSampleFile:
    ...
    def __init__(
        ...
        self.config = config
        self._name = name
        self._events = events.loc[
            :, ~events.columns.duplicated()
        ].copy()  # deduplicate columns
        self._events = MarkerCollection.renameMarkers(
            self._events
    ...
    def events(self) -> Any:
        ...
        if self._apply_gate_filter:
            gate_filter_mask = get_gate_filter_mask(
                self._labels, self.config.filter_gates
            )
        if self._apply_marker_filter:
            markers_used = self.config.markers
            if len(markers_used) == 0:
                return self._events.loc[gate_filter_mask].reset_index(
                    drop=True
                )  # use all marker -feature transfomre e.g.

        return (
            self._events.loc[gate_filter_mask]
            .reset_index(drop=True)
            .reindex(columns=markers_used, fill_value=0)
        )
    ...
    def labels(self) -> Any:
        ...
        if not [
            blastgate
            for blastgate in GateCollection.getBlastGates()
            if blastgate in self._labels.columns
        ]:
    ...
    @classmethod
    def load_from_pickle_files(
        cls,
        folder: Path,
        sample_name: str,
        config: FlowSampleConfig,
        apply_marker_filter: bool = False,
        apply_gate_filter: bool = False,
    ) -> FlowSampleFile:
        filepath_events = folder / Path(
            sample_name + cls.preloading_suffixes(config)[0]
        )
        filepath_labels = folder / Path(
            sample_name + cls.preloading_suffixes(config)[1]
        )
        return cls.load_from_pickle(
            filepath_events,
            filepath_labels,
            config,
            apply_marker_filter=apply_marker_filter,
    ...
    def load_from_pickle(
        cls,
        filepath_events: Path,
        filepath_labels: Path,
        config: FlowSampleConfig,
        apply_marker_filter: bool = False,
        apply_gate_filter: bool = False,
    ) -> FlowSampleFile:
        ...
        used to load the previously pickled pandas DF in Raw data dir.
        to further process into a graph and save in processed dir.
        """

        if not isinstance(config, FlowSampleConfig):
            raise TypeError(
                f"config must be from Type 'DataSetConfig' given {type(config)}"
            )
    ...
    def get_marker_list_idx(self) -> List[int]:
        ...
        assert len(self.get_all_markers_list()) == list(
            set(self.get_all_markers_list())
        )
    ...
    def get_class_labels(self) -> pd.Series:
        ...
        class_labels = pd.Series(
            self.config.classes_dict["other"], index=np.arange(len(self.events.index))
        )
        ...
        for key in self.config.classes_dict.keys():
            if key != "other":
                ...
                mask = mask_key & ~(
                    mask_temp
                )  # have 1 for the current key column but have not been assigned a label before already!
