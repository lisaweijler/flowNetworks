...
class BaseTrainer:
    """
    Base class for all traint ners
    """

    # use property as those are otherwise not used in base trainer.. and then i dont have to pass it over..
    @property
    ...
    def __init__(
        ...
        if base_trainer_config.monitor_config is None:
            ...
            self.mnt_metric = (
                base_trainer_config.monitor_config.data_type
                + "/"
                + base_trainer_config.monitor_config.metric_name
            )
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.mnt_mode != "off":
                try:
                    ...
                    improved = (
                        self.mnt_mode == "min"
                        and result_log[self.mnt_metric] <= self.mnt_best
                    ) or (
                        self.mnt_mode == "max"
                        and result_log[self.mnt_metric] >= self.mnt_best
                    )
    ...
    def _prepare_device(self, gpu_id):
        ...
        if n_gpu == 0:
            ...
            if gpu_id > n_gpu - 1:
                self.logger.warning(
                    "Warning: The GPU id given does not match any available GPU, set to default id = 0."
                )
                gpu_id = 0
