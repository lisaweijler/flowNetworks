...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in tqdm(
            ...
            prediction, trans_feat = self.model(current_sample)  # apply model

            loss = self.loss_ftn(prediction, current_y, trans_feat)
            ...
            with torch.no_grad():
                if epoch % self.valid_freq == 0 or epoch == self.epochs or epoch == 1:
                    for batch_idx, batch in tqdm(
                        ...
                        prediction, trans_feat = self.model(x)  # apply model

                        loss = self.loss_ftn(prediction, target, trans_feat)
